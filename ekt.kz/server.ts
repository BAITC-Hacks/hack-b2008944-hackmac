...
const CATALOG_SUMMARY = ALL_PRODUCTS.map((p) => ({
  ...
  voltage: p.voltage,
  qtyNurSultan: p.stores?.find((s) => s.name === 'Нур-Султан')?.quantity || 0,
  qtyAlmaty: p.stores?.find((s) => s.name === 'Алматы')?.quantity || 0,
  totalQty: p.quantity,
}));

const SYSTEM_INSTRUCTION = `
Ты — старший ведущий инженер-консультант интернет-магазина электротехники EKT (сайт https://nursultan.ekt.kz/, филиалы в Нур-Султане, Алматы, Шымкенте, Караганде и других городах Казахстана).
...
3. Проверять реальное наличие на складах:
   - Основной склад в Нур-Султане (ул. Бейсекбаева 24/1), Алматы, Шымкент, Караганда и др.
...
4. Товар есть в наличии на складе в Нур-Султане!`;
...
1. **027228 Legrand DRX250 MT 3ф 160А 18kA** — цена **64 920 ₸**.
   - На складе в **Нур-Султане: 8 шт.**, Алматы: 5 шт. (всего 23 шт. по РК).
...
- В наличии на складе в Нур-Султане (18 шт.).`;
...
2. **Schneider Electric RM17UAS15 (65…260 V AC/DC)** — цена **43 250 ₸**.
   ...
   - В наличии на складе в Нур-Султане!`;
...
Все указанные силовые выключатели есть в наличии в Нур-Султане!`;
...
Большой складской запас в Нур-Султане и Алматы.`;
  } else if (p.includes('астан') || p.includes('нур-султан') || p.includes('наличи')) {
    matchedIds = [515291, 515285, 25397, 45357];
    answer = `📦 **Наличие товаров на складе в Нур-Султане (ул. Бейсекбаева 24/1):**
...
3. 📦 **Проверить остатки на складе** в Нур-Султане или Алматы.
...
app.get('/api/products', (req, res) => {
  try {
    ...
    const city = (req.query.city as string) || 'nursultan';
